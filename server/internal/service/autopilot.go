...
func (s *AutopilotService) dispatchAutopilot(
	...
	if reason, code, skip := s.shouldSkipDispatch(ctx, autopilot, actorUserID); skip {
		if isRuntimeOfflineAdmissionReason(reason) {
			// MUL-2863: park the cron instead of dropping it when the
			// runtime is offline. We intentionally do NOT thread plannedAt
			// into the parked row (planned_at stays NULL): the scheduler
			// already records this occurrence as executed in
			// sys_cron_executions once DispatchAutopilotForPlan returns the
			// parked run, so the planner never re-plans it — and keeping the
			// parked row out of the (trigger_id, planned_at) idempotency key
			// stops a later DispatchAutopilotForPlan lookup from treating the
			// not-yet-complete parked run as a partial-crash row and
			// RecoverPartial-ing it. The runtime-online hook transitions this
			// same row in place, so no fresh (trigger, planned) INSERT races
...
func (s *AutopilotService) dispatchExistingPendingRun(
	...
	case "create_issue":
		...
		if err := s.dispatchCreateIssue(ctx, autopilot, &run, triggerTimezone, pgtype.UUID{}); err != nil {
			if skipped, _ := s.handleDispatchSkip(ctx, autopilot, &run, err); skipped != nil {
				return
			}
			s.failRun(ctx, run.ID, err.Error())
	...
	case "run_only":
		if err := s.dispatchRunOnly(ctx, autopilot, &run, pgtype.UUID{}); err != nil {
			if skipped, _ := s.handleDispatchSkip(ctx, autopilot, &run, err); skipped != nil {
				return
			}
			s.failRun(ctx, run.ID, err.Error())
