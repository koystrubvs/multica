...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, isolated, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier
`
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, isolated, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier FROM agent
WHERE id = $1
`
...
type UpdateAgentParams struct {
	...
	Model                    pgtype.Text `json:"model"`
	ThinkingLevel            pgtype.Text `json:"thinking_level"`
	Isolated                 pgtype.Bool `json:"isolated"`
	ServiceTier              pgtype.Text `json:"service_tier"`
	ComposioToolkitAllowlist []string    `json:"composio_toolkit_allowlist"`
}
