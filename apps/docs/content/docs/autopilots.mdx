...
| 200 | `pending_runtime` | Run-only mode and the assignee's runtime is offline: the run is parked and auto-dispatched when the runtime comes back online (MUL-2863), so the work isn't lost. |
| 200 | `skipped` | Accepted, but this run was skipped (assignee agent hard-deleted, squad archived, and so on); includes the reason. |
