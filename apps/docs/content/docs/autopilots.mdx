...
- `200 {"status":"pending_runtime","delivery_id":"…","run_id":"…","reason":"agent runtime is offline at dispatch time"}`
  — for `run_only` autopilots, the assignee's runtime is offline; the run is
  parked and will be auto-dispatched when the runtime/daemon comes back online
  (MUL-2863). The work is not lost. `create_issue` autopilots do not park — they
  still create the tracked issue so the work stays visible and can be claimed
  when the runtime returns.
- `200 {"status":"skipped","delivery_id":"…","run_id":"…","reason":"…"}`
  — admission produced a durable skipped run (assignee agent hard-deleted,
  squad archived, etc.). The reason in `failure_reason` explains why.
